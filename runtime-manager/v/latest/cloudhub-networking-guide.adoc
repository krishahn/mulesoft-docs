...
|*mule-worker-internal-myapp.cloudhub.io* |The internal IP address of the Mule workers. The IPs for this DNS record are only accessible within a customer's private link:/runtime-manager/virtual-private-cloud-about[VPC]. *These IPs are not accessible for workers running in the MuleSoft shared VPC.* Public HTTP services are exposed on `${http.port}` and `${https.port}.` Internal HTTP services are exposed on ports *8091/8092*.
|===

In certain situations, you may want to know an application’s internal IPs:

* When communicating with the worker directly within the customer's link:/runtime-manager/virtual-private-cloud-about[VPC] without sending data over the public internet
...
Additional ports can be opened inside the VPC, for example, for JMX based monitoring. In order  to do so, refer to the Firewall rules section in out link:/runtime-manager/virtual-private-cloud-firewall-rules-about[VPC documentation].
...
By configuring your own link:/runtime-manager/virtual-private-cloud-about[VPC] you can set your own link:/runtime-manager/cloudhub-dedicated-load-balancer[dedicated load balancer] through the link:/runtime-manager/anypoint-platform-cli[Anypoint Platform Command Line Interface].

== Avoiding Public Discoverability for Applications on CloudHub

If you have a link:/runtime-manager/virtual-private-cloud-about[Virtual Private Cloud] and a link:/runtime-manager/cloudhub-dedicated-load-balancer[Cloudhub Dedicated Load Balancer for VPCs], you can prevent your applications hosted in CloudHub from being publicly accessible:

. Remove the link:/runtime-manager/virtual-private-cloud-firewall-rules-about[VPC firewall rule] for ports 8081 and 8082 using:
