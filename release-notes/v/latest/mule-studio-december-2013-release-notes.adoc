...
* Having joined the API Platform family of products, the Anypoint Service Registry has a new name: link:/api-manager/[API Manager]. Further, it has has some new functionality: **link:/mule-user-guide/v/3.8/setting-up-ldap-provider-for-spring-security[LDAP Security Provider Policy]**, link:/api-manager/using-policies[Consumer Key enforcement], and *Endpoint aliasing*.
* *New and Improved CloudHub features:* New Enterprise link:/runtime-manager/virtual-private-cloud-about[Virtual Private Cloud] and support for link:/runtime-manager/managing-schedules[poll scheduling] and link:/mule-user-guide/v/3.7/batch-processing[batch processing].
...
|===
|HABITAT-1194 a|
Services, proxies, or APIs deployed to the December 2013 release on CloudHub and manually registered in API Manager using the CloudHub endpoint URL do not support runtime policies. For more details, see the link:/release-notes/api-manager-release-notes[API Manager release notes].
